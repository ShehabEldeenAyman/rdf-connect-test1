...
import { StateFactory, StateT } from "./state.js";
import { RdfStore } from "rdf-stores";
import { Member } from "./page.js";
export type Notifier<Events, S> = {
    [K in keyof Events]: (event: Events[K], state: S) => void;
};
export declare function getSubjects(store: RdfStore, predicate: Term | null, object: Term | null, graph?: Term | null): import("@rdfjs/types").Quad_Subject[];
...
export declare function enhanced_fetch(config: FetchConfig, start?: typeof fetch): typeof fetch;
export declare function limit_fetch_per_domain(fetch_f: typeof fetch, concurrent?: number): typeof fetch;
export declare function handle_basic_auth(fetch_f: typeof fetch, config: AuthConfig): typeof fetch;
export declare function retry_fetch(fetch_f: typeof fetch, partial_config: Partial<RetryConfig>): typeof fetch;
export declare function memberFromQuads(member: Term, quads: Quad[], timestampPath: Term | undefined, isVersionOfPath: Term | undefined): Member;
export {};
